 ...
 */
function init(): void {
	do_action( 'rv_starter_theme_before_init' );

	// If the composer.json isn't found, trigger a warning.
	if ( ! file_exists( RV_STARTER_THEME_PATH . 'composer.json' ) ) {
		add_action(
			'admin_notices',
			function () {
				$class = 'notice notice-error';
				/* translators: %s: the path to the plugin */
				$message = sprintf( __( 'The composer.json file was not found within %s. No classes will be loaded.', 'rv-starter-theme' ), RV_STARTER_THEME_PATH );
	...
	do_action( 'rv_starter_theme_init' );
}
 ...
 * If you're building a theme based on "rv-starter-theme", change the
 * filename of '/languages/RVStarterTheme.pot' to the name of your project.
 *
 * @return void
 */
function i18n(): void {
	load_theme_textdomain( 'rv-starter-theme', RV_STARTER_THEME_PATH . '/languages' );
}
...
function theme_setup(): void {
	...
	register_nav_menus(
		[
			'primary' => esc_html__( 'Primary Menu', 'rv-starter-theme' ),
		]
	);
}
...
function scripts(): void {
	...
	wp_enqueue_script(
		'frontend',
		RV_STARTER_THEME_TEMPLATE_URL . '/dist/js/frontend.js',
...
function admin_scripts(): void {
	wp_enqueue_script(
		'admin',
		RV_STARTER_THEME_TEMPLATE_URL . '/dist/js/admin.js',
...
function core_block_overrides(): void {
	$overrides = RV_STARTER_THEME_DIST_PATH . 'js/core-block-overrides.asset.php';
	if ( file_exists( $overrides ) ) {
		$dep = require_once $overrides;
		wp_enqueue_script(
			'core-block-overrides',
			RV_STARTER_THEME_DIST_URL . 'js/core-block-overrides.js',
			$dep['dependencies'],
			$dep['version'],
			true
		);
...
function admin_styles(): void {
	wp_enqueue_style(
		'admin-style',
		RV_STARTER_THEME_TEMPLATE_URL . '/dist/css/admin.css',
		[],
	 ...
	 * Uncomment this to use the shared.css file.
		wp_enqueue_style(
			'shared-style',
			RV_STARTER_THEME_TEMPLATE_URL . '/dist/css/shared.css',
			[],
			Utility\get_asset_info( 'shared', 'version' )
		);
	*/
...
function styles(): void {
	wp_enqueue_style(
		'styles',
		RV_STARTER_THEME_TEMPLATE_URL . '/dist/css/frontend.css',
		[],
		Utility\get_asset_info( 'frontend', 'version' )
	);
...
function assets_cleanup(): void {
	...
	if ( file_exists( RV_STARTER_THEME_BLOCK_DIST_DIR ) ) {
		$block_json_files = glob( RV_STARTER_THEME_BLOCK_DIST_DIR . '*/block.json' );
